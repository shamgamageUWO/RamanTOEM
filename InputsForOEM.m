...
x_a = [Q.Ta Q.BaJH Q.BaJL Q.CL Q.OVa Q.BaJHa Q.BaJLa Q.CLa]; % now im retrieving log of CJL
...
%%
%%
Sa_BG_JH = [zeros(1,m) (Q.CovBJH)  zeros(1,m+5)]';
Sa_BG_JL = [zeros(1,m+1) (Q.CovBJL) zeros(1,m+4)]';
Sa_BG_JHa = [zeros(1,2*m+3) (Q.CovBJHa)  0 0]';
Sa_BG_JLa = [zeros(1,2*m+4) (Q.CovBJLa) 0]';
...
Sa_CL = [zeros(1,m+2) Q.CovCL zeros(1,m+3)]; %% even here I have changed to the log version
% Sa_CHa = [zeros(1,2*m+5) Q.CovCHa 0 0 0]; %% even here I have changed to the log version
Sa_CLa = [zeros(1,2*m+5) Q.CovCLa]; %% even here I have changed to the log version

% Sa_DT_JH = [zeros(1,2*m+6)];
% Sa_DT_JL = [zeros(1,2*m+7)];
...
S_a = [Sa_Tnew Sa_BG_JH Sa_BG_JL Sa_CL' Sa_OV Sa_BG_JHa Sa_BG_JLa Sa_CLa'];% S_ainv = S_a^(-1);
