...
x_a = [Q.Ta Q.BaJH Q.BaJL Q.CL Q.OVa Q.BaJHa Q.BaJLa Q.CHa Q.CLa Q.deadtimeJH Q.deadtimeJL]; % now im retrieving log of CJL
...
%%
%%
Sa_BG_JH = [zeros(1,m) (Q.CovBJH)  zeros(1,m+8)]';
Sa_BG_JL = [zeros(1,m+1) (Q.CovBJL) zeros(1,m+7)]';
Sa_BG_JHa = [zeros(1,2*m+3) (Q.CovBJHa)  0 0 0 0 0]';
Sa_BG_JLa = [zeros(1,2*m+4) (Q.CovBJLa) 0 0 0 0]';
...
Sa_CL = [zeros(1,m+2) Q.CovCL zeros(1,m+6)]; %% even here I have changed to the log version
Sa_CHa = [zeros(1,2*m+5) Q.CovCHa 0 0 0]; %% even here I have changed to the log version
Sa_CLa = [zeros(1,2*m+6) Q.CovCLa 0 0]; %% even here I have changed to the log version

Sa_DT_JH = [zeros(1,2*m+7) Q.CovDTJH 0];
Sa_DT_JL = [zeros(1,2*m+8) Q.CovDTJL];
...
S_a = [Sa_Tnew Sa_BG_JH Sa_BG_JL Sa_CL' Sa_OV Sa_BG_JHa Sa_BG_JLa Sa_CHa' Sa_CLa' Sa_DT_JH' Sa_DT_JL'];% S_ainv = S_a^(-1);
