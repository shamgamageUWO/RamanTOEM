...
%%
%%
Sa_BG_JH = [zeros(1,m) (Q.CovBJH)  zeros(1,2*m+7)]';
Sa_BG_JL = [zeros(1,m+1) (Q.CovBJL) zeros(1,2*m+6)]';
Sa_BG_JHa = [zeros(1,2*m+3) (Q.CovBJHa) zeros(1,m+4)]';
Sa_BG_JLa = [zeros(1,2*m+4) (Q.CovBJLa) zeros(1,m+3)]';
...
Sa_CL = [zeros(1,m+2) Q.CovCL zeros(1,2*m+5)]; %% even here I have changed to the log version
Sa_CLa = [zeros(1,2*m+5) Q.CovCLa 0 0 zeros(1,m)]; %% even here I have changed to the log version

Sa_DT_JH = [zeros(1,2*m+6) Q.CovDTJH zeros(1,m+1)];
Sa_DT_JL = [zeros(1,2*m+7) Q.CovDTJL zeros(1,m)];
...
S_a = [Sa_Tnew Sa_BG_JH Sa_BG_JL Sa_CL' Sa_OV Sa_BG_JHa Sa_BG_JLa Sa_CLa' Sa_DT_JH' Sa_DT_JL' Sa_aeronew];% S_ainv = S_a^(-1);
