...
% set full raw data path
config.RawDataFolder=fullfile(config.RawDataFolder,datestr(t0,'yyyy'));
...
% P*z^2
[altv absv] = cleanAEdata(att, config);
...
%     config.localtimezone, 0, Nfile, config.ini.dASR.vertres, t0);
% 
st = storedataAE01(altv, absv, S4, config.TreatedDataFolder, ...
    config.localtimezone, 0, Nfile, config.ini.dABS.vertres, t0);

st = storedataBulletin01(S0, wv, Temp, ASRatio, Ext, config, t0);
