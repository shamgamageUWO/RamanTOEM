function Temperature = Temperature02(data, binave, precision, calibration)
% 
...
JLsum       = data.JL.(what).Signal(:,Shots>0) * Shots(Shots>0) / ScaleFactor;
...
JHsum       = data.JH.(what).Signal(:,Shots>0) * Shots(Shots>0) / ScaleFactor;
...
if sum(~isnan(SignalRatio_relerr))~=0
