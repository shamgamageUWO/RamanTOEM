function st = storedataASR01(data, ASRatio, TreatedDataFolder, ...
                                    localTime, fileaveraging, numfiles, ...
                                        BinsOrVector, StartTime)
                                    
% status = storedataASR01(data, ASRatio, TreatedDataFolder, ...
%                           localTime, fileaveraging, numfiles, ...
%                                 BinsOrVector, StartTime)
% Writes to text file:
% data              - data structure from LoadLicel function 
% ASRatio           - Result structure from AerosolScatteringRatio02.m 
...
% don't write file, if no valid data
if sum(~isnan(ASRatio.profile))==0
    st=0;
    return
end
    ...
    filename        = ['Ralmo_ASR_',datestr(StartTime, 'yymmdd_HHMM'),'.txt']; % to be changed with middle time
    %
    ...
    %
    % Header 
    h01	= sprintf('RALMO - Aerosol Scattering Ratio \n');
    ...
    h12 = sprintf('Calibration: %f\n',ASRatio.cal);
    h14 = sprintf('OK flag: %d\n\n', ASRatio.okflag*1);
    h15 = sprintf('Alt[m],  ASR,      AbsErr, RelErr\n\n');
    %
    ...
    ind = zeros(size(ASRatio.z));
    ind(1:ASRatio.errind(3)) = 1;
    ind = logical(ind);
    % Stores data in 'file pipe'
    y=[ASRatio.z(ind)';ASRatio.profile(ind)';ASRatio.abserr(ind)'; ASRatio.relerr(ind)'];
