...
for i=1:length(configTmp.t)
    ...
    if ~isempty(S2) & ~isempty(S3)
        j = j + 1;
        dzRaw = S2.N2.Combined.Range(2) - S2.N2.Combined.Range(1);
        if S2.N2.Combined.Shots ~= 1
            jshots = sum(S2.N2.Combined.Shots);
            y2HzRaw = clight ./ (2.*(jshots./1800).*Rate.*dzRaw);
            WVcts = S2.WV.Combined.Signal ./ (y2HzRaw./1e6);
            N2cts = S2.N2.Combined.Signal ./ (y2HzRaw./1e6);
            wct = sum(WVcts');
            n2ct = sum(N2cts');
        else
            jshots = S2.N2.Combined.Shots;
            y2HzRaw = clight ./ (2.*(jshots./1800).*Rate.*dzRaw);
            wct = S2.WV.Combined.Signal ./ (y2HzRaw./1e6);
            n2ct = S2.N2.Combined.Signal ./ (y2HzRaw./1e6);
        end
        zASR = S3.z;
...
%        timeEnd(j) = (S3.starttime+S3.stoptime)./2; %datenum(S2.GlobalParameters.Start);
        zCounts = S2.N2.Combined.Range;
    end
end

asr.z = zASR;
asr.asr = asrRaw;
save S2S3201503050030combined.mat asr zCounts WVcounts N2counts shots timeEnd
