function [effi_stoke,effi_anti,shift_O2_as,shift_O2_s,diff_O2s,diff_O2as]= RR_differentialO2_JH_transmission(J,T)
...
effi_stoke = [0.37619836,0.98166707,0.53912801,0.0511171];
effi_anti= [0.34586496,0.94294973,0.59074973,0.08572700];
...
%                            Xas(k) = ((J(k)-2)*((J(k)-2)-1))/(2*(J(k)-2)-1);
                    ...
                    for i =1: length(T)
                        for k = 1: length(J)
                           ...
                           diff_O2s(i,k) = ((effi_stoke(k).* Const_O2*gi*B_O2*(v0+shift_O2_s(k))^4 * Xs(k)* exp(-Es(k)/ (kb*T(i))))./T(i)).*(10^-4);% SI m^2
