...
% asr = Q.asr-1;
aero = dfacaero.*Q.asr; 
% Saero1 = (dfacaero.*ones(n1,1)').^2;
% Saero2 = (dfacaero.*ones(n3,1)').^2;%
% Saero = [Saero1 Saero1 Saero2 Saero2];
% S_aero = diag(Saero);
                    Saero1 = (aero(end-n1+1:end)).^2;
                    Saero2 = (aero(1:n3)).^2;
                    Saero = [Saero1 Saero1 Saero2 Saero2];
                    S_aero = diag(Saero);

% beta = dfacbetamol.*Q.beta_mol;
%                     Sbeta1 = (beta(end+1-n1:end)).^2;
%                     Sbeta2 = (beta(1:n3)).^2;
%                     Sbeta = [  Sbeta1  Sbeta1  Sbeta2  Sbeta2];
%                     S_beta = diag(Sbeta);
%                  
...
% S_b.Sxbeta = X.G*R1.Jbeta*S_beta*R1.Jbeta'*X.G';
...
% beta = diag(S_b.Sxbeta);

total_err_T = sqrt( X.eo(1:m).^2 + P(1:m)  + Rc(1:m)+ Ra(1:m) + Air(1:m) + Aero(1:m) );%+ beta(1:m));
...
% Error.Beta_ErrorT =sqrt(beta(1:m));
...
% plot(sqrt(beta(1:m)),Q.Zret./1000,'b')
semilogx(total_err_T,Q.Zret/1000,'black')
xlabel('Temperature Uncertainty (K)')
ylabel('Altitude (km)')
legend('Statistical','Pressure','R','Ra','Sigma Rayleigh','ASR','Total Error')
...
% Errors for Overlap
total_err_OV = sqrt( X.eo(m+4:2*m+3).^2 + P(m+4:2*m+3) + Rc(m+4:2*m+3)+ Ra(m+4:2*m+3) + Air(m+4:2*m+3) + Aero(m+4:2*m+3));
...
% Error.Beta_ErrorT =sqrt(beta(m+4:2*m+3)).*1e2;
...
% plot(sqrt(beta(m+4:2*m+3)).*1e2,Q.Zret./1000,'b')
...
% Aerosol Error
total_err_ae = sqrt( X.eo(2*m+9:end).^2 + P(2*m+9:end) + Rc(2*m+9:end)+ Ra(2*m+9:end) + Air(2*m+9:end) + Aero(2*m+9:end) );
...
% Error.Beta_Errorae =sqrt(beta(2*m+9:end)).*1e2;
...
% semilogx(sqrt(beta(2*m+9:end)).*1e2,Q.Zret./1000,'b')
