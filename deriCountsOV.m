function [dOVJHa,dOVJLa] = deriCountsOV(j,Q,x,forwardmodelTraman)
...
% N = 2*m+6 ;
    ...
    [y_JLa,y_JHa]=forwardmodelTraman(Q,x);
...
%     m=length(Q.Zret);
%     xa=x(1:m);
    dn = x(m+1+j).*1e-8; % this can go anything smaller than 0.1 even for higher temperatures works ok
    xpert = x;
    if x(j) == 0 % trap for tau's where tau(1) = 0
        dn = 1.e-8 .* x(m+1+j+1);
    end
    xpert(m+1+j) = x(m+1+j) + dn;
%     Xpert= [xpert x(end-2) x(end-1) x(end)];

    [y_JLa_dT,y_JHa_dT]=forwardmodelTraman(Q,xpert);
